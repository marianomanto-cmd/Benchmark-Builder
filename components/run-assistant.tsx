  ...
  return (
    <>
        ...
        {open && (
          ...
          >
            <div style={{ display: "flex", alignItems: "center", gap: 9, padding: "14px 16px", borderBottom: "1px solid var(--border)" }}>
              ...
              <div style={{ flex: 1 }}>
                <div style={{ fontSize: 13, fontWeight: 600, color: "var(--text)" }}>{t("ra.title")}</div>
                <div style={{ fontSize: 10, fontFamily: "var(--font-mono)", color: "var(--text-faint)" }}>{t("ra.subtitle")}</div>
              </div>
              <button type="button" onClick={() => setOpen(false)} aria-label={t("common.close")} style={{ border: "none", background: "transparent", color: "var(--text-muted)", cursor: "pointer", display: "inline-flex" }}><X size={16} /></button>
            </div>

            <div style={{ flex: 1, overflowY: "auto", padding: 14, display: "flex", flexDirection: "column", gap: 10, minHeight: 0 }}>
              {msgs.length === 0 && (
                <div style={{ fontSize: 13, color: "var(--text-muted)", lineHeight: "19px", fontStyle: "italic" }}>
                  {t("ra.examples")}
                </div>
              )}
              {msgs.map((m, i) => (
                <div key={i} style={{ alignSelf: m.role === "user" ? "flex-end" : "flex-start", maxWidth: "88%", padding: "9px 12px", borderRadius: 12, fontSize: 13, lineHeight: "19px", background: m.role === "user" ? "var(--accent)" : "var(--surface-2)", color: m.role === "user" ? "var(--accent-ink)" : "var(--text)", border: m.role === "bot" ? "1px solid var(--border)" : "none" }}>
                  ...
                  {m.outOfScope && (
                    <div style={{ marginTop: 8 }}>
                      <Link href="/" style={{ display: "inline-flex", alignItems: "center", gap: 6, fontSize: 12, fontWeight: 600, color: "var(--accent)", textDecoration: "none" }}><Home size={13} /> {t("ra.newSearch")}</Link>
                    </div>
                  )}
                </div>
              ))}
      ...
      >
        ...
        <span className="bb-hide-sm">{t("ra.cta")}</span>
      </button>
    </>
  );
}
