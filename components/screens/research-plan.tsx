...
function ParamRow({ k, v, pos }: { k: string; v: string; pos?: boolean }) {
  return (
    <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", padding: "8px 12px", background: "var(--surface)", border: "1px solid var(--border)", borderRadius: "var(--r-sm)" }}>
      <span style={{ fontSize: 11, color: "var(--text-muted)", fontFamily: "var(--font-mono)", textTransform: "uppercase", letterSpacing: ".06em" }}>{k}</span>
      <span style={{ fontSize: 12, fontWeight: 500, color: pos ? "var(--success)" : "var(--text)", fontFamily: "var(--font-mono)" }}>{v}</span>
    </div>
  );
}
  ...
  return (
    <ScreenShell breadcrumb={["Proyectos", "Cartagena · Q2 2026", "Plan de research"]} badges={<BBBadge tone="warn" size="sm">esperando aprobación</BBBadge>} runMeta="generado por IA · costo estimado USD 1,84">
      <div style={{ display: "grid", gridTemplateColumns: "1.5fr 1fr", gap: 18 }}>
        <div>
          <div className="t-micro" style={{ color: "var(--accent)" }}>STEP 2 / 4 · PLAN PROPUESTO</div>
          <div className="t-h1" style={{ marginTop: 6, color: "var(--text)" }}>Antes de gastar tokens, mostrámelo.</div>
          <div className="t-body" style={{ color: "var(--text-muted)", marginTop: 8, maxWidth: 540 }}>
            La IA propone qué scrapear, dónde y por qué. Vos editás las fuentes, ajustás filtros y aprobás antes del run.
          </div>

          <div style={{ marginTop: 20, background: "var(--surface)", border: "1px solid var(--border)", borderRadius: "var(--r-md)", padding: 18, boxShadow: "var(--sh-1)" }}>
            ...
            <div style={{ marginTop: 14, display: "flex", flexDirection: "column" }}>
              {sources.map((r, i) => (
                <div key={i} style={{ display: "flex", alignItems: "center", gap: 14, padding: "12px 0", borderTop: i ? "1px solid var(--border)" : "none" }}>
                  <span style={{ width: 18, height: 18, borderRadius: 3, border: "1px solid var(--accent)", background: "var(--accent)", display: "flex", alignItems: "center", justifyContent: "center", color: "#fff", flexShrink: 0 }}><Ic.check s={11} /></span>
                  <PlatformBadge platform={r[0]} size="md" />
                  <div style={{ flex: "0 0 130px", fontSize: 13, fontWeight: 500, color: "var(--text)" }}>{r[1]}</div>
                  <div style={{ flex: 1, fontSize: 11, color: "var(--text-muted)", fontFamily: "var(--font-mono)" }}>{r[2].slice(0, 3).join(" · ")}{r[2].length > 3 ? ` +${r[2].length - 3}` : ""}</div>
                  <div style={{ flex: "0 0 90px", fontSize: 11, color: "var(--text-muted)", fontFamily: "var(--font-mono)" }}>{r[3]}</div>
                  <div style={{ flex: "0 0 70px", fontSize: 11, color: "var(--text)", fontFamily: "var(--font-mono)", textAlign: "right", fontWeight: 500 }}>{r[4]}</div>
                  <Btn kind="ghost" size="sm" icon={<Ic.filter s={10} />}>Editar</Btn>
                </div>
              ))}
            </div>
            <div style={{ marginTop: 8, paddingTop: 14, borderTop: "1px solid var(--border)", display: "flex", alignItems: "center", gap: 14 }}>
              <span className="t-micro" style={{ color: "var(--text-muted)" }}>TOTAL ESTIMADO</span>
              <div style={{ flex: 1 }} />
              <span style={{ fontFamily: "var(--font-mono)", fontSize: 11, color: "var(--text-muted)" }}>~2.126 piezas · 4 min 12 s</span>
              <span style={{ fontFamily: "var(--font-mono)", fontSize: 18, fontWeight: 600, color: "var(--text)" }}>USD 1,84</span>
            </div>
          </div>
        </div>

        <div style={{ display: "flex", flexDirection: "column", gap: 14 }}>
          <div style={{ background: "var(--surface)", border: "1px solid var(--border)", borderRadius: "var(--r-md)", padding: 18, boxShadow: "var(--sh-1)" }}>
            <div className="t-micro">RAZONAMIENTO · IA</div>
            <div style={{ marginTop: 12, fontSize: 13, lineHeight: "21px", color: "var(--text)", textWrap: "pretty" }}>
          ...
          <div style={{ background: "var(--surface-2)", border: "1px solid var(--border)", borderRadius: "var(--r-md)", padding: 18 }}>
            ...
            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12, marginTop: 12 }}>
              ...
              <ParamRow k="Análisis sentim." v="Claude · Anthropic" />
            </div>
          </div>
