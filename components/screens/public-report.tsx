...
// (Descargar PDF), the shared <ReportDocView> on the white sheet. Branding is
// Phatia by default; the white-label pass (task 6) will recolor/relabel this.
export function PublicReport({ doc }: { doc: ReportDoc }) {
  return (
    <div style={{ minHeight: "100dvh", background: "var(--surface-2)", display: "flex", flexDirection: "column", alignItems: "center", padding: "clamp(10px, 3vw, 28px)" }}>
      ...
      <article className="bb-print" style={{ width: "min(900px, 100%)", background: "#fff", boxShadow: "var(--sh-4)", borderRadius: 6, padding: "clamp(26px, 6vw, 76px)", fontFamily: "var(--font-serif)", color: "var(--n900)" }}>
        <header style={{ display: "flex", alignItems: "center", justifyContent: "space-between", gap: 10, flexWrap: "wrap", paddingBottom: 14, borderBottom: "1px solid var(--n200)", marginBottom: "clamp(18px, 4vw, 30px)" }}>
          <span style={{ display: "inline-flex", alignItems: "center", gap: 8, fontFamily: "var(--font-mono)", fontSize: 10.5, letterSpacing: ".12em", textTransform: "uppercase", color: "var(--n500)" }}>
            <span style={{ width: 5, height: 18, background: "var(--sa-base)", display: "inline-block", borderRadius: 1 }} /> Phatia · reporte compartido
          </span>
