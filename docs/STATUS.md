...
**Onboarding del vórtice + fixes de QA (sesión 15/jun):** **animación de onboarding** (3 pasos, metáfora de vórtice) que aparece al tocar **"Comencemos"** (usuario nuevo, sin login; `MarketingHero`) — `components/screens/onboarding.tsx` + `onboarding.module.css`: canvas 2D (modo por paso: 1 coral/contexto, 2 neutro/datos, 3 atenuado + tarjeta de reporte), etiquetas "absorbidas" al núcleo, dots 1·2·3, "Saltar" y "Siguiente"/"¡Entendido!" → **al final abre el wizard** (`onFinish` → `setWizard(true)`). Responsive (100dvh, `min(82vw,48vh,380px)`), reduced-motion estático, copy sin «ruido» (usa "información/datos"). i18n `onb.*` (ES/EN/PT). **Fixes de QA** (de la review): nav **mobile** ahora se ve (`.mobileMenu` ya no queda `display:none`); **Cockpit bento** (`.bb-bento`), **galería** y **roadmap FODA** colapsan/scrollean en mobile (antes desbordaban); `key={i}` → keys estables en overview; `dash.qa1-6` agregadas a EN/PT.
