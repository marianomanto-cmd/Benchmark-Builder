...
**Logos de marca + recap de valor en el wizard (sesión 15/jun):** (1) **Avatares de cuenta con logo** — `components/account-avatar.tsx` (`AccountAvatar`): muestra el logo de la marca (Clearbit `logo.clearbit.com/{domain}`, mapeado por **slug** en `lib/accounts.ts` → `BRAND_DOMAINS`, así funciona aunque el directorio cacheado no tenga el campo) sobre tile claro; **fallback** a la inicial tinte-acento si no hay dominio o el logo falla (`onError`). Usado en el dashboard, `account-view` y el dashboard compacto del home. Cuentas creadas por el usuario → inicial. (2) **Paso "Confirmar" del wizard**: además del marco en vivo, ahora hay un **recap de valor** — "Qué hacemos con esto" (frase dinámica con marca/competidores/mercados/fuentes/ventana) + "Qué vas a recibir" (las 4 secciones del reporte, reusa `deliv.r*n`) + cierre de valor ("un único reporte editable, listo para tu cliente; sin planillas, sin adivinar"). i18n `wizard.confirm*`. (3) El overlay del onboarding usa el **mismo scrim que el wizard** (`--bg 78%` + blur 12) — sin salto de opacidad en la transición.
