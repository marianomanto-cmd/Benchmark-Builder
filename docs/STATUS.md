...
lib/
  ...
  ai/ data.ts demo.ts **demo-cases.ts (registro multi-caso por slug)** **i18n.ts (ES/EN/PT)** view-models.ts database.types.ts supabase/ format.ts
...
**i18n (ES/EN/PT) (sesión 14/jun):** base liviana en `lib/i18n.ts` (dicc. plano + `t()`), `I18nProvider` (cookie `phema_locale` + localStorage; SSR lee la cookie en `layout.tsx` y setea `<html lang>`), **selector de idioma** (`components/language-select.tsx`) en el **header del shell** y en la **nav del landing** (`router.refresh()` al cambiar). **Traducido:** landing público (nav, hero, process, faq, footer) + frame de la app (sidebar/header) + **wizard completo** (pasos, campos, scope, redes, ventana, confirmación, **popup del asistente** y los textos del asistente en `suggest.ts` — `assistFor`/`recommendationsFor` con `locale`; `/api/wizard/assist` recibe `locale` y responde en ese idioma). **Pendiente (próxima tanda):** pantallas del run (overview/live-feed/comparativa/galería/FODA/settings/editor/proyectos/runs) y **contenido seed** (`demo.ts`/`demo-cases.ts`: análisis, insights, FODA, hero; menciones quedan en idioma original — realista para social listening).
