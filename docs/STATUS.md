...
> Documento de contexto para consultar en otra conversación. Última actualización: 2026‑06‑14 (rediseño Claude Design del **área de run**: Overview y FODA con **toggle de 4 vistas** + **vórtice** en "Qué hace" del home; monetización por **créditos**: config único + modal de suscripción + saldo en header/dashboard + asistente del run gasta créditos; directorio editable CRUD; orquestación Wizard Assistant/Haiku + Planner/QuerySpec + Grok web; wizard 3 pasos; seed multi-caso; actores automáticos). Producto: **Phatia** (repo: Benchmark-Builder).
