...
- **Nada scrollea de costado: ni la página, ni un contenedor de adentro, ni
  una tabla dentro de su card.** Ninguna celda va en `nowrap` salvo lo que no
  se puede partir —un número de documento, una fecha, un monto—: el texto
  envuelve y se corta con `line-clamp`, porque `truncate` convierte el largo
  del dato en ancho mínimo de la columna. No
...
| **El login mandaba usuario y contraseña por la URL** si el envío ocurría antes de hidratar —el HTML llegó, el bundle no, que con mala señal en el mostrador pasa—: el `<form>` no tenía `method`, así que el submit nativo es un GET, y los campos se llaman `username` y `password` para el gestor de contraseñas. Verificado con JavaScript apagado: `…/login?username=recepcion&password=Clave-Real-2026`, a la vista de quien esté del otro lado del mostrador, en el historial y en el log de accesos | `method="post"` como red y el botón deshabilitado hasta hidratar como camino: con el único submit deshabilitado el navegador tampoco manda con ⏎ |
| **`?desde=` era un redirect abierto.** `destinoSeguro()` chequeaba el string a mano, y el parser de URL del navegador BORRA los tabs y saltos de línea antes de resolver: `?desde=/%09/evil.example.com` pasaba los tres chequeos y terminaba en `//evil.example.com`. Y el salto ocurría DESPUÉS del login exitoso, que es cuando alguien está más dispuesto a escribir sus datos en una pantalla que dice Smile Lab | Se resuelve contra el propio origen y se comparan orígenes: lo que se compara es el resultado del mismo parser que después navega |
| **Cualquiera del equipo podía BORRAR la ficha de otro**, la del admin incluida: la policy es `for all` —incluye DELETE— y `guard_es_admin()` colgaba sólo de INSERT y UPDATE. El único freno era accidental (la FK `on delete restrict` protegía a quien ya había firmado un presupuesto, a nadie más) | Las fichas no se borran desde una sesión del equipo: se dan de baja. Y ninguna operación puede dejar al consultorio sin un administrador con acceso (migración 22) |
| **Una ficha dada de baja seguía siendo administradora**: ni `es_admin()` en SQL ni `getPerfil()` miraban `activo`, así que pasaba por `exigirAdmin()` y podía deshacer su propia baja. El ban de GoTrue impide renovar el token, no invalida el que la persona ya tiene abierto | `es_admin()` y `getPerfil()` miran `activo`, y el layout manda al login con el motivo |
| **La búsqueda no encontraba a la mayoría de los pacientes.** `ilike` en Postgres distingue acentos y los pacientes se guardan «Apellido, Nombre»: «Gomez» no traía ninguno de los 31 presupuestos de «Gómez, Renata», ni «Suarez» los 6 de «Suárez, Martín», y copiar el nombre del propio listado y pegarlo tampoco, porque la coma no puede viajar en el filtro de PostgREST. En el picker del wizard el costo es peor: no encuentra, ofrece «Crear paciente», y la agenda queda con dos fichas de la misma persona | Columna `busqueda` generada y normalizada en la base, y palabra por palabra en cualquier orden. El mismo criterio del lado del cliente (`coincide()`), porque los comboboxes filtran en memoria (migración 23) |
...
| Arreglado el alto, soltar afuera **seguía** escribiendo: `closestCorners` era la reserva también con mouse, y esa función siempre devuelve algo — inventa el destino más cercano aunque el puntero esté 150px afuera, sobre el fondo de la página. Lo encontró el escéptico midiendo los rincones uno por uno | Con puntero manda el puntero y nada más: soltar donde no hay zona es cancelar. `closestCorners` queda para el teclado, que no tiene puntero. Con eso, la rama defensiva de `alSoltar` y el anuncio «fuera del tablero» dejan de ser código inalcanzable |
...
| **La tabla de la Home se cortaba la columna «Estado» con los datos reales del consultorio.** `truncate` es `white-space: nowrap`, así que cada columna le pide al navegador el largo ENTERO de su texto —topeado por `max-w`— y esa suma es el mínimo de la tabla: con «TRABAJADORES PASTELEROS, CONFITEROS, PIZZEROS Y AL» y «Corona de zirconio + reconstrucción de muñón con fibras» daban 1.408px dentro de una caja de 1.114. Con el seed nunca se veía: sus nombres son cortos y no llegaban al tope, así que mi barrido daba verde con el bug vivo | Las celdas ENVUELVEN y se cortan con `line-clamp-2`: el mínimo de una columna pasa a ser su palabra más larga (~100px) y no la frase. Y los dos montos van en una celda —«a cargo» arriba, el total debajo sólo si difieren—, que es una columna menos y se lee mejor. La tabla queda en 1.114 |
| **El barrido medía el scroll de la página, no si la tabla se sale de SU caja.** La card tiene `overflow-hidden`: una tabla más ancha que ella no mueve la página ni pinta barra, simplemente corta las columnas de la derecha sin que nada avise. Así pasaron cuatro tablas rotas en 768–1024 | El barrido compara `table.scrollWidth` contra el `clientWidth` de su caja, y además inyecta en cada campo de texto variable el dato más largo que existe en la base del consultorio. 108 chequeos: 12 pantallas × 9 anchos, de 320 a 1920 |
| `Equipo` a 768: «Contraseña» y «Dar de baja» en una línea son 245px de ancho mínimo para esa columna sola, y la tabla no entraba | Los botones envuelven. Los dos siguen con texto: dar de baja es destructivo y nunca va como ícono solo |
| `Biblioteca · pacientes` a 1024: un mail es UNA palabra, así que su ancho entero (~200px) era el mínimo de la columna «Paciente» —315px— y la tabla se salía de la card | `overflow-wrap: anywhere` en el mail. El mínimo pasa a ser el del nombre |
| El detalle abría dos columnas en `md`: la principal quedaba en 354px —768 menos las sangrías y los 330 del costado— y la tabla de prestaciones necesita 424. Se cortaba «A cargo», que es el número del documento | Dos columnas recién en `lg`. Apilado hay ancho de sobra y el seguimiento queda abajo, que es donde se mira después de leer el presupuesto |
| El tablero se arrastraba de costado en `md` y `lg`: cinco columnas de 980px de piso dentro de una caja de 768. Arrastrar de costado un tablero donde además hay que arrastrar tarjetas son dos gestos que se pisan | El piso baja a 940 y el tablero aparece en `lg`, donde justo entra (960 de caja). Debajo, el mismo camino que en el teléfono: el listado de Home filtrado por los estados en juego |
| En la tarjeta del tablero el monto iba al lado del nombre y es `shrink-0`: se llevaba ~90px y en una columna de 182 al nombre le quedaban 60. El tablero mostraba «Abril, Ro…», «Meghrun…», «Gómez,…» — y un tablero donde no se sabe de quién es la tarjeta no sirve para lo único que hace | El nombre ocupa el ancho entero y el monto baja a la fila de la obra social. La etiqueta «A cargo» queda en `sr-only`: en el tablero todo monto es el que queda a cargo |
| El wordmark de la topbar se partía en «Smile» / «Lab» a 768. Ponerle `whitespace-nowrap` hacía que la fila pidiera 784px y la página ENTERA scrolleara 16px de costado, en todas las pantallas a la vez | No se parte, y abajo de `lg` va el símbolo solo (`textoDesde="lg"`), que es lo que hace cualquier barra angosta. El nombre sigue en el `sr-only` |
| **La flecha de los `Select` se salía del control.** El `span` que Radix pinta para el valor es `inline`, así que no le entra un `overflow` y su ancho completo empujaba a la flecha: en el pipeline, «Todos los profesionales» son 152px de texto en un control de 180 que deja 132, y la flecha terminaba 9px FUERA del borde — la de al lado, 2px. Dos flechas a distinta distancia del borde es lo que se lee como desalineado. El comentario del trigger prometía puntos suspensivos desde el día uno y no había ni un `truncate` | El valor va envuelto en un `flex-1 min-w-0 truncate` dentro de `SelectTrigger`: el texto cede lugar y la flecha queda clavada a 13px del borde en TODOS los selects. Medido en los diez de la app: mismo margen, centradas, sin recorte |
| Los filtros del pipeline mostraban «Todos los profesio…» | Dicen «Profesional» y «Obra social», la misma decisión que ya había tomado la barra de la Home: los vecinos de la fila son sustantivos sueltos («Emitido»). El «Todos los…» sigue adentro de la lista, que es donde significa «sacá el filtro» |
...
- **El barrido corre con el peor dato que existe en la base**, no con el
  seed: el nombre de paciente, la obra social y la prestación más largos del
  consultorio, inyectados a la vez en cada campo de texto variable. Medir con
  el seed daba verde con la tabla de la Home rota: sus nombres son cortos y
  nunca llegaban al tope de truncado, que es justo donde aparece el problema.
- **Una tabla más ancha que su card no mueve la página.** La card tiene
  `overflow-hidden`, así que corta las columnas de la derecha sin barra y sin
  aviso. El barrido compara `table.scrollWidth` contra el `clientWidth` de su
  caja, además del scroll de la página y el de cada contenedor.
